...
5. daisy-toolsを開始
6. コード化合物が食べられ、細胞分裂する様子を観察

観察していると、`cell`ディレクトリ内に同時に存在する細胞の数は最大で2つになります。試しに`code`ディレクトリのコード化合物の数を増やして同様の実験を行うと、`cell`ディレクトリに同時に存在する細胞の数が増える事が確認できます。この様に、`code`ディレクトリに配置するコード化合物の数によって細胞分裂をある程度制限することができます。
