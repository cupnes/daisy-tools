...
- ステータス：
  - 未調査
- 現象：
  - 確認している限りは5日以上の実行で、メインプロセス([`dsy-sysenv`](../bin/README.md#dsy-sysenv))が標準出力へ以下のようにメッセージを出力して突然終了する事がある
    ```
    ff3fff74e6 の適応度が 90 になりました。
    ff3fff74e6 がbin/run: 36 行: 20394 Segmentation fault      (コアダンプ) bin/dsy-sysenv
    ```
- 影響：
  - segv時、書き込み途中だったファイルは壊れているかもしれない
  - 影響する可能性があるのは、`cell`・`code`ディレクトリ以下のファイル
- 発生した場合の対処：
  - 対処1：[`cell-fitness-logger`](../bin/README.md#cell-fitness-logger)の停止
    - [`run`](../bin/README.md#run)コマンドで実行していた場合、メインプロセスと並列で起動される`cell-fitness-logger`プロセスが停止されないまま残ってしまっているので、`kill`コマンド等で停止させる
  - 対処2：`cell`・`code`ディレクトリの対処
    - Linuxの標準出力はある程度バッファされていたりするため、`Segmentation fault`のメッセージが出ている行の処理がsegvの時に実行中であったとは限らない
    - そのため、可能であれば、`cell`・`code`の両ディレクトリ内は一旦捨てて、実験をやり直すのが良い
    - 実験をやり直すのが難しい場合は、以下の方法で`cell`・`code`の両ディレクトリ内のファイルをチェックし、壊れているファイルのみ捨てると良い
      - `cell`ディレクトリ：
        - 全ファイルを`dsy-dump-cell`でチェックする
        - 簡易チェックとしては、それぞれを`dsy-dump-cell`でダンプしてみてエラーにならなければ良い
        - しっかりチェックするなら、それぞれの内容が意図通りであるかもチェックする
      - `code`ディレクトリ：
        - ファイルフォーマット通りかをチェックする
        - 簡易チェックとしては、ファイルサイズが16バイトであるかをチェックすると良い
        - しっかりチェックするなら、[「コード/データ化合物ファイル」のファイルフォーマット](file-format.md#%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB%E3%83%95%E3%82%A9%E3%83%BC%E3%83%9E%E3%83%83%E3%83%88-1)通りであるかをチェックする
- 再現性：
  - あり
- 再現手順：
  - 「['A'を出力する実行バイナリ生成(改良版)](http://yuma.ohgami.jp/Daisy-Tools_Laboratory_Report/03_exp_res.html#h3-3)」の実験(`samples/setup-sample puta`でセットアップされる実験)を[エンドレスモード](#%E9%81%A9%E5%BF%9C%E5%BA%A6100%E3%81%AE%E7%B4%B0%E8%83%9E%E3%81%8C%E7%94%9F%E3%81%BE%E3%82%8C%E3%81%A6%E3%82%82dsy-sysenv%E3%82%92%E7%B5%82%E4%BA%86%E3%81%95%E3%81%9B%E3%81%AA%E3%81%84%E3%81%A7%E5%8B%95%E4%BD%9C%E3%81%95%E3%81%9B%E7%B6%9A%E3%81%91%E3%82%8B%E6%96%B9%E6%B3%95%E3%82%A8%E3%83%B3%E3%83%89%E3%83%AC%E3%82%B9%E3%83%A2%E3%83%BC%E3%83%89)で実行し続けていると、確認している限りは、5日以上の実行で再現する模様
  - その他の実験でも再現するかは不明
  - 別の実験をやっていた際は10数日実行し続けてもこの現象に陥らなかったこともあったが、本件の原因がわかっていないのでなんとも言えない
